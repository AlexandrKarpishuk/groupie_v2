...
class WorkoutFriendsVC : UIViewController {
    
    fileprivate var commentsCount = [999, 0, 1]
...
//        NotificationCenter.default.addObserver(self, selector:#selector(updateWorkouts), name: NSNotification.Name(rawValue: Constants.Notifications.WorkoutsUpdated), object: nil)
  ...
  //      self.workoutTableView?.estimatedRowHeight = 180
    ...
    func needUpdateWorkouts() {
        self.readFromCache()
        DispatchQueue.main.async {
            if let visibleCellsIndexPath = self.workoutTableView!.indexPathsForVisibleRows {
    ...
    func readFromCache() {
        self.workouts = WorkoutsManager.shared.workoutsFriends.sorted(by: { (info1:WorkoutInfo, info2:WorkoutInfo) -> Bool in
    ...
    fileprivate func updateWorkouts() {
        if (self.workouts.count == 0) {
    ...
    //        let unused:[WorkoutInfo] = notify.userInfo![WorkoutsManager.INFO_UNUSED]! as! [WorkoutInfo]
            ...
            let friendsNew = sortedNew.filter({ (info:WorkoutInfo) -> Bool in
                WorkoutsManager.shared.isFriendsWorkout(info)
            })
            for index in 0..<friendsNew.count {
            ...
            if (toUpdatePaths.count > 0 || toInsertPaths.count > 0) {
                DispatchQueue.main.async { [weak self] in
                    ...
                    if let strongSelf = self {
                        strongSelf.workouts.insert(contentsOf: friendsNew, at: 0)
                        for i in 0..<friendsNew.count {
...
extension WorkoutFriendsVC: UITableViewDelegate {
...
extension WorkoutFriendsVC: UITableViewDataSource {
    ...
    func tableView(_ tableView:UITableView, cellForRowAt indexPath:IndexPath) -> UITableViewCell {
        ...
        cell.onLeave = { [weak self, weak tableView] (workout: WorkoutInfo?) in
            if (workout != nil) {
                WorkoutsManager.shared.Leave(workout: workout!, onSuccess: { [weak tableView] (workout:WorkoutInfo) in
