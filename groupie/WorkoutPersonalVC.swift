...
class WorkoutPersonalVC : UIViewController {
    
    fileprivate var commentsCount = [1, 2, 3]
    ...
    @IBOutlet weak var noWorkoutsYet: UIView?
    @IBOutlet weak var noWorkoutsTitle: UILabel?
    @IBOutlet weak var noWorkoutsDescription: UILabel?
   ...
   //     self.workoutTableView?.estimatedRowHeight = 180
    ...
    func didPost() {
        DispatchQueue.main.async {
            ...
            if (self.workouts.count - oldPostsCount == 1) {
    ...
    func readFromCache() {
        self.workouts = WorkoutsManager.shared.workoutsPersonal.sorted(by: { (info1:WorkoutInfo, info2:WorkoutInfo) -> Bool in
        ...
        if (self.workouts.count == 0) {
            UIView.animate(withDuration: 0.24, animations: { 
                self.noWorkoutsYet?.alpha = 1.0
            }, completion: { (Bool) in
                self.noWorkoutsYet?.alpha = 1.0
                self.noWorkoutsYet?.isUserInteractionEnabled = true
            })
        } else {
            UIView.animate(withDuration: 0.24, animations: { 
                self.noWorkoutsYet?.alpha = 0.0
            }, completion: { (Bool) in
                self.noWorkoutsYet?.alpha = 0.0
                self.noWorkoutsYet?.isUserInteractionEnabled = false
            })
        }
    }
    ...
    //        let unchanged:[WorkoutInfo] = notify.userInfo![WorkoutsManager.INFO_UNCHANGED]!  as! [WorkoutInfo]
            let updated = notify.userInfo![WorkoutsManager.INFO_UPDATED]! as! [WorkoutInfo]
            let new:[WorkoutInfo] = notify.userInfo![WorkoutsManager.INFO_NEW]! as! [WorkoutInfo]
    //        let unused:[WorkoutInfo] = notify.userInfo![WorkoutsManager.INFO_UNUSED]! as! [WorkoutInfo]
            ...
            var personalNew = [WorkoutInfo]()
            for index in 0..<personalNew.count {
            ...
            if let user = ServerManager.shared.currentUser {
                personalNew = sortedNew.filter({ (info:WorkoutInfo) -> Bool in
                    info.isUserWorkout(user.username)
                })
            }
            ...
            if (toUpdatePaths.count > 0 || toInsertPaths.count > 0) {
                DispatchQueue.main.async { [weak self] in
                    self?.workoutTableView?.beginUpdates()
                    if let strongSelf = self {
                        strongSelf.workouts.insert(contentsOf: personalNew, at: 0)
                        for i in 0..<personalNew.count {
...
extension WorkoutPersonalVC: UITableViewDelegate {
...
extension WorkoutPersonalVC: UITableViewDataSource {
    ...
    func tableView(_ tableView:UITableView, cellForRowAt indexPath:IndexPath) -> UITableViewCell {
        ...
        cell.onLike = { [weak self] (workout: WorkoutInfo?) in
            if (workout != nil) {
                if (LikesManager.shared.isLiked(workout: workout)) {
                    LikesManager.shared.dislike(workout: workout!, onSuccess: { [weak self] (workout:WorkoutInfo, Int64) in
                        DispatchQueue.main.async { [weak self] in
                            if let visibleCells = self?.workoutTableView?.indexPathsForVisibleRows {
                                self?.workoutTableView?.reloadRows(at: visibleCells, with: UITableViewRowAnimation.none)
                            }
                        }
                    ...
                    LikesManager.shared.like(workout: workout!, onSuccess: { (workout:WorkoutInfo, Int64) in
                        DispatchQueue.main.async { [weak self] in
                            if let visibleCells = self?.workoutTableView?.indexPathsForVisibleRows {
                                self?.workoutTableView?.reloadRows(at: visibleCells, with: UITableViewRowAnimation.none)
                            }
        ...
        cell.onLeave = { [weak self, weak tableView] (workout: WorkoutInfo?) in
            if (workout != nil) {
                WorkoutsManager.shared.Leave(workout: workout!, onSuccess: { (WorkoutInfo) in
                    DispatchQueue.main.async {
                        tableView?.beginUpdates()
        ...
        cell.onEdit = { [weak self, weak tableView] (workout: WorkoutInfo?) in
            ...
            self?.p_editVC?.onInviteHandler = {
                ...
                if (inviteFriendVC != nil) {
                    ...
                    if let friends = self?.parseFriends(users: self?.p_editVC?.withWhomUsers) {
                    ...
                    inviteFriendVC!.onInviteHandler = { [weak self] (selected: [UserInfo]) in
                        ...
                        self?.p_editVC?.withWhomUsers = selected
                        self?.p_editVC?.showWithWhomUsers()
        ...
        cell.onAvatar = { [weak self] (workout: WorkoutInfo?) in
            if (workout?.organizer_name != nil) {
                ...
                UsersManager.shared.getUser(userName: workout!.organizer_name, completed: { (user:UserInfo) in
        ...
        cell.onUserTapped = { [weak self] (userNick: String?) in
            if (userNick != nil) {
               ...
               UsersManager.shared.getUser(userName: userNick!, completed: { (user:UserInfo) in
